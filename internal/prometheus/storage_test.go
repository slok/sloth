...
func TestIOWriterGroupedRulesYAMLRepoStore(t *testing.T) {
	tests := map[string]struct {
		...
		"Having a single SLO recording rule should render correctly.": {
			slos: []prometheus.StorageSLO{
				{
...
groups:
- name: sloth-slo-recordings-test1
  rules:
  - record: test:record
    expr: test-expr
    labels:
...
groups:
- name: sloth-slo-recordings-testa
  rules:
  - record: test:record-a1
    expr: test-expr-a1
    labels:
...
- name: sloth-slo-alerts-testa
  ...
  - alert: testAlertA2
    ...
    annotations:
      test-annot: a-2
- name: sloth-slo-recordings-testb
  rules:
  - record: test:record-b1
    expr: test-expr-b1
    labels:
...
`,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			repo := prometheus.NewIOWriterGroupedRulesYAMLRepo(&gotYAML)
